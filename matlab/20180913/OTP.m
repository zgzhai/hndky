...
for m=1:nl
    ...
    if rongliang(m,1)>90            %   修改了判定条件大于90
    panduantiaojian=1;
    end
end
%% 判断是否进行负荷削减的条件
     ...
     if(z<0.000000001)     
        ...
        %% fprintf('    已找到最优解!\n');    此处删除    
         ...
         %% 输出结果       
          %fprintf('基向量为:'); X
         if f<=0
             fprintf('此种状态下不需要削减负荷，可参考负荷转移建议！ \n') ; 
         elseif f>0
             fprintf('此种状态下需进行负荷转移以优化电网静态稳定性，可参考负荷转移优化建议！\n') ;  
         end
         ...
         if(B\A(:,k)<=0)          % 如果B\A(;,k)中的每一个分量都小于零。。
               ...
               fprintf('     \n 请加强重载线路及重载变压器的运行监控，可参考负荷转移优化建议！\n');  
         else
               b1=B\b';
               temp=inf;
               for i=1:m
...
%% 以下是if panduantiaojian==1的else
    ...
    fprintf('此状态下不需要削减负荷，请继续监控！\n');
end
%% 负荷转移优化建议
%若出现过载或超过90%负荷率，则需要负荷转移

fuhezhuanyi=zeros(nl,1);
for i=1:nl
    fuhezhuanyi(i,1)=Line(i,5);
end

for m=1:nl   
  %且本体风险A、B,能关联正常周期负荷能力吗？
          
    if rongliang(m,1)>90
        fuhezhuanyi(m,1)=2*fuhezhuanyi(m,1)*(rongliang(m,1)-80);
        fprintf('建议此处变电站至少向附近站点转移%2.2fMW的负荷，负荷转移后请重新校核主变风险！\n',fuhezhuanyi(m,1));
    else
        fuhezhuanyi(m,1)=-1;
%如果相等只输出一个结果
    end
      fprintf('建议此处变电站至少向附近站点转移%2.2fMW的负荷，负荷转移后请重新校核主变本体风险！\n',fuhezhuanyi(m,1));
  %且本体风险C、D, fprintf(myf,'无需进行负荷转移，建议根据变压器风险预测结果控制风险！ \n',fuhezhuanyi(m,1));
  %接口能不能打包到一起?
end
