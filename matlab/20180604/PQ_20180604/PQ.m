...
%% 加入的容量一列
[rongliang]=xlsread('pqinput.xls','S3:S100');
...
sum2=sum2+U(pq(m))*U(n)*(G(pq(m),n)*sin(a(pq(m))-a(n))-B(pq(m),n)*cos(a(pq(m))-a(n)));  
      ...
      if max2<=e 
         ...
         if kp==0
             ...
             k=k+1;
% if k>300
%      break  跳出循环，结束整个PQ分解法程序，并显示“PQ分解法无法收敛，是否转入潮流的线性估算？”
%      “是”启动直流潮流法，“否”结束运行
         end
      else %否则计算电压修正量
      Uq=U;
...
zhilushiji=zeros(y,1);
for m=1:y
    if XT(m)==0 %线路支路
        zhilushiji(m,1)=(real(Sij(m))^2+imag(Sij(m))^2)^0.5/(3^0.5);%线路实际负载电流
    else  % 变压器支路
        zhilushiji(m,1)=(real(Sij(m))^2+imag(Sij(m))^2)^0.5;        %变压器支路负载功率
    end
    rongliang(m,1)=zhilushiji(m,1)/rongliang(m,1);
    rongliang(m,1)=100*rongliang(m,1);
    if rongliang(m,1)>60
    fprintf(fid,'                                  节点%d到节点%d支路负载率较大有越限风险，负载率约为:  %2.2f%%  \n',I(m),J(m),rongliang(m,1));
    else
    fprintf(fid,'                                  此状态下节点%d到节点%d支路负荷率无越限风险   \n',I(m),J(m));    
    end
end
for m=1:x
    if U(m)>1.05||U(m)<0.95
    fprintf(fid,'                                   第%d节点电压有越限风险，上下限超过5%%  \n',m);
    else
    fprintf(fid,'                                   此状态下第%d节点电压无越限风险   \n',m);       
    end
end 
