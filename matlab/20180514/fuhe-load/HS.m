...
Y_temdata=1;           %是否存在变压器额定负荷（100%）的温升数据和60-80%负荷的温升数据；若有，则Y_temdata=0，否则Y_temdata=1；
...
%%%%%%%%在线数据%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        T_amb=G(:,1);                        %读取环境温度.
        I_H_current=G(:,2);                  %读取高压负荷电流.
        I_M_current=G(:,3);                  %读取中压负荷电流.
        I_L_current=G(:,4);                  %读取低压负荷电流.
        
        T_top_C=G(:,5);                    %读取顶层油温测量值.           %%%%实际运行变压器没有热点温度监测装置.                                                             
        p_sun=G(:,6);                      %读取太阳日辐射功率.
        %%%%%%%%太阳辐射功率%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %%%调整计算间隔进行插值%%%%%%% 
        ...
        I_H_current_1=zeros(1,C_bei*num);                      %负载电流间隔调整数组
...
%       Wind_1=zeros(1,C_bei*num);                           %风速间隔调整数组
        ...
        for i=1:num
            for j=1:C_bei
                ...
                I_H_current_1(j+C_bei*(i-1))=I_H_current(i);
