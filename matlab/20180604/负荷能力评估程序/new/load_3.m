%  function y=load_3      %正常周期负荷能力评估

n_iters=96;                  %迭代次数，一天24小时，每小时采4个点,
IC1=xlsread('state_data');    %读取变压器状态评分值
IC=IC1(9,1);

Klimit_fu=2;               %辅助设备容量等级约束
Tlimit_top=115;              %顶层油温约束
Tlimit_hs=140;               %热点温度约束
Llimit_L=100*86400;          %相对寿命损失约束
...
Tap=9;                 %输入高压侧实际分接位置.
n=0.33;                 %输入计算指数（0.25~0.5） 一般情况下，对于AN类型的变压器n取值接近0.33 ,对于AF类型的变压器n取值接近0.5
n1=0.5;                 %输入计算指数（0.25~1.0） 一般情况下，对于ON类型的变压器m取值接近0.5 对于OD类型的变压器m取值接近1.0
...
switch (kind)
    case 1
        ...
        I_H_DC=E(8,1);                                %读取温升试验下变压器中压侧负载电流（A）.
...
I_H_current_2=zeros(1,n_load);                    %用于计算的负荷率数组
K2=zeros(1,n_load);                               %用于存储负荷率的数组
T2=zeros(1,n_load);                               %用于存储运行时间的数组
for i1=1:n_load
...
for r=1:n_load
    ...
    I_H_current=I_H_current_2(r); 
    Time=0;                             %用于累计时间
    T_amb=Con(1,1);                     %最高环境温度或一天的平均温度,或者一天的温度曲线。
    p_sun=0;                            %忽略日照辐射影响 
for i1=1:n_iters
         T_amb_1(i1)=T_amb;
         I_H_current_1(i1)=I_H_current;
         p_sun_1(i1)=p_sun; 
end
%%%%行矩阵转变成列矩阵%%%%%%%
            T_amb=T_amb_1';
...
% %每15min进行一次校正一次计算得到的变压器温度.
     
for k=1:(2*6*interval)                              %每次的时间间隔
...
%     u_p=exp(2797.3/(T_wnd_now+273))/exp(2797.3/(T_wnd_rate+273));
    ...
    [x2,y]=ode23(T_wnd,t,T_wnd_now);                %（求解微分方程，T_wnd_now相当于y，t相当于x）
    T_wnd_now=y(5);                               %10s后的平均绕组温度
...
if Flimit_IC>1
    ...
    K2(r)=(Flimit_IC-1)*(K2(r)-0.01);
else
    K2(r)=Flimit_IC*(K2(r)-0.01);
end
