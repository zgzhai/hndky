...
T_amb=Con(:,1);                     %最高环境温度或一天的平均温度,或者一天的温度曲线。
...
%         T_top_G1=zeros(1,num);                                %用于记录顶层油温度数据的数组.      
%         T_hs_G1=zeros(1,num);                                 %用于记录热点温度数据的数组.
%         V_G1=zeros(1,num);                                    %用于记录变压器相对老化速率的数组.
%%%%%%%%%变压器当前各温度值%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
legend ('热点温度估算值','顶层油温估算值','相对老化速率')            %,,'热点温度测量值')
%title('变压器热路模型热点温度计算')
ylabel('温度/摄氏度')
xlabel('时间/分钟')
